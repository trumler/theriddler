...
'Keep going — you’re almost consistently terrible.',
  ...
  Future<void> _onMenuTap(int index) async {
    ...
    // Now hide the menu after fade out is completed
    setState(() {
      showMenu = false;
      showIntro = false;
      showQuiz = false;
      showFinished = false;
      quizOpacity = 0.0;  // Explicitly ensure quizOpacity starts at 0.0
    });

    // Add a slight delay before fading in the quiz
  ...
  Future<void> _fadeToTask(int newIndex) async {
    ...
    if (newIndex < tasks.length) {
      ...
      // Mark expedition quiz as completed for trophy unlock
      final prefs = await SharedPreferences.getInstance();
      await prefs.setBool('expedition_completed', true);
    }
    await Future.delayed(Duration(milliseconds: 50));
    setState(() {
      quizOpacity = (showQuiz || showFinished) ? 1.0 : 0.0;
              ...
              : showFinished
                  ? Center(
                      child: AnimatedOpacity(
                        ...
                        duration: quizFadeDuration,
                        child: Text(
                          'You have now completed\n THE EXPEDITION. GOOD JOB! The expedition trophy is now unlocked!',
                          style: TextStyle(color: Colors.white, fontSize: 24),
                          textAlign: TextAlign.center,
                        ),
                      ),
